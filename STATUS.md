...
- Every planned phase is shipped, including Phase 7 (NL -> plan translator). The only open items are stand-fixture-blocked: `ptz_controller` (no PTZ camera), `schedule_descriptor_get` (no schedule fixture), and the high-risk 5F-B2 slice held out of scope. Archived `vmda_query` and `search_events` (archived events) are both live-verified against real recorded data on camera 1.
