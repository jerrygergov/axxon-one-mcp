...
**Current coverage:** **283 / 361 gRPC RPCs `tested-pass`** (24 pending, 54 fixture-warn), **969 / 969** unit tests. The canonical per-RPC ledger is [`docs/api-audit/capability-vs-coverage-2026-06-05.md`](docs/api-audit/capability-vs-coverage-2026-06-05.md) + `docs/api-audit/mcp-corpus/api_methods.json`; that file, not this handoff, is the source of truth for live status. Since Phase 8 the project ran a per-service gRPC coverage sweep (Phases 9–46): Phase 44 physical/hardware (Telemetry/HeatMap/Media), Phase 45 MediaService `ConnectEndpoint` (live mic → speaker), and Phase 46 Videowall + Logic reversible writes (`register/change/set-control/unregister`, `change_config/change_counters/counter_action`, all live round-tripped through their shipped MCP modules). The remaining 24 pending are mostly destructive/irreversible (license mutations, backup/restore, domain topology, cloud bind, SMTP/GSM notifiers); the 54 fixture-warn are live-probed but stand-blocked (missing driver, server defect, or absent fixture). The historical Phase 1–8 narrative below is kept for context.
