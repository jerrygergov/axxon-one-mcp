...
- Every planned phase is shipped, including Phase 7 (NL -> plan translator). The only open items are stand-fixture-blocked: `ptz_controller` (no PTZ camera), `schedule_descriptor_get` (no schedule fixture), archived `vmda_query` (no archiving store), and the high-risk 5F-B2 slice held out of scope.
...
Test suite baseline on `main`: 668 / 668 passing.
