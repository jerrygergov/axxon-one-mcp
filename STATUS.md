...
- Every planned phase is shipped, including Phase 7 (NL -> plan translator). The only open items are stand-fixture-blocked: `ptz_controller` (no PTZ camera), `schedule_descriptor_get` (no schedule fixture), `recent_events` (no archived events on this stand), and the high-risk 5F-B2 slice held out of scope. Archived `vmda_query` is now live-verified against real recorded objects on camera 1.
...
4. **Stand-config gaps that cannot be closed via the API** (documented, not code work): `schedule_descriptor_get` (no schedule descriptor object on the stand) and `recent_events` (this stand persists no events to its queryable EventHistory DB; returns data on any stand with event archiving). Archived `vmda_query` is no longer in this list: it is live-verified against real recorded objects on camera 1.
