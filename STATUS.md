...
- Every planned phase is shipped, AND all three former "open items" are now closed (Phase 8, 2026-06-05, 682/682 tests). PTZ: `axxon_mcp_ptz.py` (`--enable-ptz`, 17 TelemetryService tools) — the "no PTZ camera" claim was a false negative from reading only the filtered inventory; the full config graph has a real PTZ device (`DeviceIpint.53/TelemetryControl.0`), and AbsoluteMove was live-verified moving the device and restoring it. 5F-B2: `security_user_credential_lifecycle` adds reversible ephemeral user login+password mutation, live-verified create->change->verify->remove with zero leftovers. Schedules: proven a hard API limitation (no schedule RPC/message/object anywhere in PDF, protos, or the live config tree; desktop-client authored only). Archived `vmda_query` and `search_events` are also live-verified against real recorded data on camera 1.
