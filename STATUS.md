...
**Current coverage:** **283 / 361 gRPC RPCs `tested-pass`** (20 pending, 58 fixture-warn), **969 / 969** unit tests. The canonical per-RPC ledger is [`docs/api-audit/capability-vs-coverage-2026-06-05.md`](docs/api-audit/capability-vs-coverage-2026-06-05.md) + `docs/api-audit/mcp-corpus/api_methods.json`; that file, not this handoff, is the source of truth for live status. Since Phase 8 the project ran a per-service gRPC coverage sweep (Phases 9–47): Phase 44 physical/hardware (Telemetry/HeatMap/Media), Phase 45 MediaService `ConnectEndpoint` (live mic → speaker), Phase 46 Videowall + Logic reversible writes (`register/change/set-control/unregister`, `change_config/change_counters/counter_action`, all live round-tripped through their shipped MCP modules), and Phase 47 StateControl/TagAndTrack probe-and-classify (4 stand-walled methods moved to fixture-warn). The remaining 20 pending are mostly destructive/irreversible (license mutations, backup/restore, domain topology, cloud bind, SMTP/GSM notifiers); the 58 fixture-warn are live-probed but stand-blocked (missing driver, server defect, or absent fixture). The historical Phase 1–8 narrative below is kept for context.

**Branch state:** `main` includes everything through Phase 6B plus Phase 5H (metadata/VMDA search) and Phase 7 (NL -> plan translator). On 2026-06-04 a full 0->6B live re-verification pass ran, then a destructive live gap-closure pass exercised every non-PTZ mutating path, Phase 5H shipped the live object-track search, and Phase 7 shipped the recipe assembler (live-verified: 3 ephemeral recipes round-trip assemble->validate->apply->rollback). On 2026-06-04 the archived `vmda_query` was corrected to the documented MomentQuest `ExecuteQuery` binding and live-verified against real recorded object data on camera 1 (5 intervals with object bounding boxes; a 14-day sweep found 3931 intervals on day-5). On 2026-06-05 the archived-events gap was closed: it was a false conclusion (`ReadCount`=0, the same wrong read the VMDA gap had). `search_events` actually returns thousands of events; two real bugs were fixed (the `mmexport.ExportEvent` Any body crashed `message_to_dict`, and the numeric epoch-1900 timestamp returned 0), live-verified at 300 events including `ExportEvent`. On 2026-06-05 Phase 8 closed the last three items: PTZ tools are API-verified against a virtual telemetry endpoint (`DeviceIpint.53/TelemetryControl.0`, found via the full config graph; AbsoluteMove is accepted and the stored pan/tilt coordinates update, but there is no physical PTZ camera on the stand, so this verifies the TelemetryService gRPC surface, not motorized hardware), 5F-B2 gained a reversible `security_user_credential_lifecycle` (live create->change login+password->verify->remove, zero leftovers), and schedules are proven a hard API limitation (no schedule surface anywhere). Test suite: 682/682. Continue on `main` unless the user asks for a new worktree. Every planned phase is built and every gap is either closed or documented as a hard API limit.
...
- Every planned phase is shipped, AND all three former "open items" are now closed (Phase 8, 2026-06-05, 682/682 tests). PTZ: `axxon_mcp_ptz.py` (`--enable-ptz`, 16 `ptz_*` tools: 15 TelemetryService control tools plus `ptz_connect_axxon_profile`) — there is no physical PTZ camera on the stand, but the full config graph exposes a virtual telemetry endpoint (`DeviceIpint.53/TelemetryControl.0`, absent from the filtered inventory) that exercises the TelemetryService gRPC surface: AbsoluteMove is accepted and the stored pan/tilt coordinates update (verified by move + readback + restore). This verifies the API, not motorized hardware; presets are empty and zoom is not honored. 5F-B2: `security_user_credential_lifecycle` adds reversible ephemeral user login+password mutation, live-verified create->change->verify->remove with zero leftovers. Schedules: proven a hard API limitation (no schedule RPC/message/object anywhere in PDF, protos, or the live config tree; desktop-client authored only). Archived `vmda_query` and `search_events` are also live-verified against real recorded data on camera 1.
- Phase 5B (PTZ) is shipped and API-verified (Phase 8): the stand has no physical PTZ camera, but `DeviceIpint.53/TelemetryControl.0` (a virtual telemetry endpoint, found via the full config graph) responds to the documented TelemetryService. `axxon_mcp_ptz.py` drives it; AbsoluteMove is accepted and stored coordinates update on readback, then restore. This is gRPC-surface verification, not motorized-hardware verification (no presets, zoom not honored).
...
- Phase 5H (metadata/VMDA search) is shipped: `list_vmda_sources`, `live_track_sample` (live `MetadataService.PullMetadata` tracklets — verified live, caught 14 moving objects on camera 1's tracker), and `vmda_query` (archived `VMDAService.ExecuteQuery` MomentQuest motion-in-area) behind `--enable-metadata`. This is the gRPC equivalent of the desktop "Metadata search". The query binds `access_point` to the VMDA database (`*/VMDA_DB.N/Database`, auto-discovered), `camera_ID` to the host-relative detector source, `schema_ID="vmda_schema"`, and a MomentQuest `query` string with `language="EVENT_BASIC"` (per Integration APIs 3.0 pages 295/447). Live-verified against real recorded objects on camera 1's tracker `AVDetector.1` (returns intervals with object bounding boxes).
- Phase 7 (NL -> plan translator) is shipped behind `--enable-translator`: `assemble_recipe` turns an English intent into an ordered sequence of existing operator workflows (never invents API shapes; unsupported intents such as PTZ/role/permission return a structured `unsupported_intent` gap), `validate_recipe` dry-runs each step via `OperatorRegistry.plan` and aggregates risk/approvals/gaps, and `explain_recipe` renders an offline risk + rollback + wall-clock preview. Live-verified: 3 ephemeral recipes (`create_camera`, `create_macro`, `external_event_inject`) round-trip assemble->validate->apply->verify->rollback->verify-absent on the stand.
...
Test suite baseline: 969 / 969 passing (as of Phase 46, 2026-06-08).
...
2. **Confirm the toolchain is ready:**
   ...
   # current main baseline: Ran 682 tests OK
   ```
3. **Set demo-stand env for any live verification:**
   ...
   ```
4. **Phases 1-6B, 5H, and 7 are shipped; every planned phase is built.** All of 5D/5E/5F are closed against the live stand (the reversible 5F-B2 role-edit slice shipped; the rest of 5F-B2 stays deferred, and `schedule_descriptor_get` needs the stand-side schedule fixture above). 6A: the multi-language renderer seam is in place and 13 template kinds support Python + Node/TypeScript (26 bundles) — the 8 base templates plus `alarm_responder`, `scheduled_exporter`, `ml_detector_bridge`, `dashboard_backend`, and `plugin_scaffold`; 8 generated bundles were live-verified against the demo stand. 6B: the `PartnerKit` (scaffold/lint/package) shipped with reference plugins in `customer-templates/`, verified live scaffold->run (lists the stand's cameras). A destructive live gap-closure pass (`.agent/tasks/phase-6-gap-closure/`) exercised every non-PTZ mutating path on the stand: `ml_detector_bridge` raised real events, `alarm_responder` reviewed 5 real alerts (fixing a `SV_NOTICE`->`SV_ALARM` severity bug), and admin/alarms/bookmark/operator mutations all passed with clean rollback. 5H: metadata/VMDA object-track search (`tools/axxon_mcp_metadata.py`) shipped behind `--enable-metadata` — `live_track_sample` is live-verified (caught 14 moving objects), and `vmda_query` (ExecuteQueryTyped MotionInArea) is proto-faithful and returns data on any stand that records metadata. 7: the NL -> plan translator (`tools/axxon_mcp_translator.py`, `--enable-translator`) composes existing operator workflows from English intent and was live-verified with 3 ephemeral recipes round-tripping assemble->validate->apply->rollback.

A destructive live gap-closure pass (2026-06-04, `00ec63d`, see `.agent/tasks/phase-6-gap-closure/`) exercised every non-PTZ mutating path against the stand: `ml_detector_bridge` raised real `Event1` events into `DetectorEx.1` (`raised: 2`); `alarm_responder` Began+Completed review on 5 real alerts (which surfaced and fixed a template bug — the server rejects `SV_NOTICE(2)`, so completion now uses `SV_ALARM(4)`); admin mutations PASS=6; alarms raise/begin/continue/cancel ok; gRPC bookmark Create/Get/Delete PASS; and the operator full apply/verify/rollback cycle ran all 8 ephemeral workflows. Remaining in Phase 6: only `ptz_controller`, blocked on a live PTZ-camera fixture (C# remains a future layer). One confirmed hard stand-config gap that cannot be closed via the API: `schedule_descriptor_get` (no schedule descriptor object exists on the stand; schedules are desktop-client authored). The earlier `recent_events`/archived-events "no events" claim was wrong (it came from `ReadCount`=0); `search_events` returns thousands of events once the `ExportEvent` Any-body decode and the timestamp format are fixed (see `.agent/tasks/phase-5i-archived-events/`).
...
| 5B — PTZ / telemetry control | ✅ shipped + live-verified | 17 (`--enable-ptz`: discover, session lifecycle, move/zoom/focus/iris, absolute move, presets, aux ops) | `tools/axxon_mcp_ptz.py`, `.agent/tasks/phase-8-finish-all/` |
...
| 5F-B1/B2 — Security/admin mutations | ✅ shipped (B2 complete) | 7 workflows (incl. `security_user_credential_lifecycle`) | `docs/api-audit/phase-5f-b-admin-mutation-smoke-latest.md` |
...
| 6A — Authoring kit expansion (Python + Node) | ✅ shipped | 13 template kinds Python+Node (26 bundles); 5 new kinds added (alarm_responder, scheduled_exporter, ml_detector_bridge, dashboard_backend, plugin_scaffold); 8 bundles live-verified on stand | `tools/templates/*.tmpl`, `tools/tests/test_axxon_mcp_generator_6a*.py`, `.agent/tasks/phase-6a-*` |
...
| 7 — NL → plan translator | ✅ shipped + live-verified | 3 (`--enable-translator`: assemble/validate/explain recipe) | `tools/axxon_mcp_translator.py`, `.agent/tasks/phase-7-nl-translator/` |
| 8 — Final gap closure | ✅ shipped + live-verified | PTZ live motion + reversible user-credential lifecycle + schedule impossibility proof | `.agent/tasks/phase-8-finish-all/` |

---

## What's left in the roadmap

See [the roadmap](docs/superpowers/specs/2026-05-16-axxon-mcp-full-coverage-roadmap.md) for the full breakdown. Every planned phase is shipped, including **Phase 7**. What follows is the deferred/fixture-blocked tail plus completed-phase notes.

1. **Phase 7 — NL → plan translator (SHIPPED, `.agent/tasks/phase-7-nl-translator/`).** `assemble_recipe`, `validate_recipe`, `explain_recipe` behind `--enable-translator`; composes existing operator workflows from natural-language intent, refuses to invent API shapes, and was live-verified with 3 ephemeral recipes round-tripping apply->rollback on the stand.
...
4. **Stand-config gaps that cannot be closed via the API** (documented, not code work): `schedule_descriptor_get` (no schedule descriptor object on the stand). Archived `vmda_query` and `search_events` (archived events) are no longer in this list: both are live-verified against real recorded data on camera 1.
