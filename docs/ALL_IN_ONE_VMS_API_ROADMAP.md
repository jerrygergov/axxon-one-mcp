...
| Actual all-enabled MCP tools | 326 | 321 server-local decorators plus 5 delegated translator tools |
| Advertised capability groups | 53 | `CAPABILITY_GROUPS` in `tools/axxon_mcp_server.py` |
...
| Layouts, maps, videowalls, client UI | Layout CRUD, map CRUD/images/markers/providers, videowall register/change/control data, Client HTTP API screen/layout control. | Strong server-side layout/map/wall coverage; Phase 5 `client_api` adds a read-only Client HTTP API preflight + fixture-needed operation catalog (port 8888 closed on the current stand). | Promote the Client HTTP API screen/layout operations to apply-ready once a reachable client-local HTTP API target exists. |
...
| HTTP, WebSocket, embeddable video | `/v1` endpoint lookup, legacy HTTP examples, HTTP `/grpc`, WebSocket camera events, Client HTTP API, embeddable video component. | Strong docs/generator support; Phase 5 added the read-only `web_api` group (embeddable-component URL/command helpers + bounded WebSocket-event probe/sample, live-verified against a reachable Web server on port 80) and the read-only `client_api` group (Client HTTP API preflight + fixture-needed operation catalog). | Promote `client_api` operations from fixture-needed to apply-ready once a reachable Client HTTP API target (default port 8888) exists; decode application-level WebSocket event payloads when the subscribe protocol is documented. |
...
| `DevicesCatalog` | Vendors, devices, traits. | Covered by `devices_catalog`; consumed by `bulk_onboarding` validation. | Expand wizard-style driver selection hints from catalog traits. |
...
3. **Intent polish:** first-class `notification` and `tag_and_track` groups still pending. Phase 1 added the read-only `site_graph` group, Phase 2 added the approval-gated `export` group, Phase 3 added approval-gated `bulk_onboarding`, Phase 4 added approval-gated `detector_playbooks`, and Phase 5 added the read-only `web_api` (live-reachable Web server) and `client_api` (preflight + fixture-needed, since the Client HTTP API port 8888 is closed on the current stand) groups; future work should consume them from planners and generators rather than rejoining inventory ad hoc.
